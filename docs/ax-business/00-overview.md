...
> 브랜드명은 **하우제로(HowZero)로 확정**됐으나(→ §4), 각 문서 본문의 `하우제로` 토큰 일괄 치환은 아직 수행 전이다.
...
| # | 항목 | 상태 | 다음 액션 |
|---|---|---|---|
| 1 | **`하우제로` 토큰 치환** | 브랜드명은 하우제로로 확정됐으나 04~11 문서 본문의 토큰 일괄 치환 미수행 | 치환 실행 + AX 서비스용 서브도메인/경로(예: /ax) 전략은 Phase 2에서 결정 (03 §9) |
