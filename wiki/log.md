...
- 요청: howzero AX 사업 자산이 새 레포 `zerowater-may/howzero-ax`로 이전·push 완료. 원 레포에서 이전분 삭제 + 포인터 정리.
...
- 핵심: 이 vault에는 이제 Commerce 축만 남는다. AX 전환 관련 질문/작업은 새 레포 `zerowater-may/howzero-ax`의 `persona/`·`strategy/`·`reference/`를 참조한다.
