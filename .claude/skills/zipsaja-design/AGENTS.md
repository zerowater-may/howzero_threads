# 집사자 (zipsaja) Design — AGENTS.md

> 이 폴더의 AI 작업 지침 원본은 `AGENTS.md`다. `CLAUDE.md`는 이 파일을 읽으라는 포인터로만 둔다.
...
- **서버 경로**: `/opt/proptech/`
- **DB**: `postgresql://proptech:<server-env-password>@localhost:5432/proptech_db`
  - 외부 노출 안 됨. SSH로 들어가서 `psql` 실행 또는 ssh-tunnel.
  - 실제 비밀번호는 추적 문서에 적지 말고 서버 env에서 확인한다.
...
```bash
ssh root@151.245.106.86 "set -a && source /opt/proptech/.env && set +a && psql \"$DATABASE_URL\" -c \"
