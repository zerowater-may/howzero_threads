---
name: carousel
description: 인스타그램/소셜미디어 카드뉴스 캐러셀을 HTML로 생성하고 Puppeteer로 1080x1440 PNG로 자동 추출합니다. 필요하면 cardnews-generator식 훅/구성/시안 브리프를 먼저 만든 뒤, 기존 HTML 렌더러로 최종 PNG를 안정적으로 뽑습니다.
---
...
1. **브랜드/스타일 결정** — 어떤 브랜드/스타일인지 파악하고 해당 프리셋 파일을 읽는다:

   | 키워드 | 프리셋 | 파일 | 특징 |
   |--------|--------|------|------|
   | `howzero`, `하우제로` | 하우제로 | `brands/howzero/README.md` + `docs/persona-howzero.md` | 크림 배경 + 파스텔 형광펜 + Nanum Pen Script, 얼리머조리티 창업가 톤 |
   | `braveyong`, `용감한용팀장`, `용팀장` | 용감한용팀장 | `brands/braveyong/README.md` | 크림/다크 + 붉은·골드 + Black Han Sans, 팩트 폭격·도발 톤 |
   | `mkt`, `mkt.co.kr`, `마케터스클럽`, `브루탈 미니멀`, `미니멀 마케터` | mkt | `brands/mkt/README.md` | 밝은 회색 + 주황 불릿 1점 + Pretendard Black, 조용한 존댓말 인사이트 톤 |
   | `zipsaja`, `집사자`, `go.nyangee`, `부린이`, `내집마련` | 집사자 | `brands/zipsaja/README.md` + `brands/zipsaja/workflow.md` + `brands/zipsaja/assets/` | 크림 베이지 + 오렌지 물감 pill + Jua 둥근 손글씨 + 손그림 마스코트, 친근 반말 부동산 큐레이션 톤. **Pill/타이틀에 SVG filter 물감 효과 필수.** Nano Banana V2 파이프라인(`scripts/nano_carousel`)으로 손그림 템플릿 생성 + HTML 텍스트 오버레이 하이브리드 지원 (`brands/zipsaja/workflow.md` 참조) |
   | 그 외 | — | 사용자에게 톤/브랜드 질문 | — |
...
### Step 0.5: 아이디어/시안 브리지 (필요 시)

최종 산출물은 계속 `slides.html` + Puppeteer PNG다. 다만 사용자가 `시안`, `훅`, `후킹`, `더 잘 팔리게`, `랜딩 유입`, `용팀장`, `cardnews-generator`를 언급하거나, 주제만 있고 구성이 약하면 먼저 `references/cardnews-ideation-bridge.md`를 읽고 **아이디어 브리프**를 만든다.

브리지의 역할:

- `cardnews-generator`의 장점인 훅 뱅크, 10장 흐름, 비주얼 방향, 커버 시안 사고방식을 앞단에 사용한다.
- `idea-brief.md`에 선택한 훅, 카드별 역할, 카드 문구, 레이아웃 후보, CTA를 정리한다.
- 이미지 생성은 최종물이 아니다. 사용자가 명시적으로 요청한 경우에만 커버 또는 핵심 2~3장 시안용으로 사용한다.
- 최종 게시용 문구와 PNG는 반드시 기존 HTML/CSS 렌더링 단계에서 확정한다.
...
**금지 레이아웃 — 좌/우 분할 (L3 SIDE)**
16:9 가로 영상 프레임을 슬라이드의 세로 절반에 넣으면 상하가 강제 크롭되어 인물과 자막이 잘려 나간다. 가독성이 크게 떨어지므로 **좌우 분할 레이아웃은 쓰지 말 것**. 대신 L1(TOP), L4(BOTTOM), L5(SANDWICH) 중 선택.
...
```
1: HERO            ← 훅
2: L1 TOP          ← 권위 + stat (이미지 + 텍스트)
3: L4 BOTTOM       ← 숫자 / 공식
4: L6 OVERLAY      ← 규칙 상세 (비주얼 임팩트)
5: HERO            ← 반전/전환점
6: L5 SANDWICH     ← 2단계 설명
7: L1 TOP          ← 실전 팁 1
8: L5 SANDWICH     ← 실전 팁 2 (사례)
9: L4 BOTTOM       ← 체크리스트
10: HERO           ← CTA
```

이대로 복사할 필요는 없고, 콘텐츠에 맞춰 풀에서 고르되 **연속 중복 금지 + L1 과사용 금지 + L3 SIDE 절대 금지**만 지켜라.
...
- 브랜드 폴더 구조: `brands/{name}/README.md` (프리셋 스펙) + `brands/{name}/assets/` (마스코트/로고 등 이미지 에셋)
- **하우제로**: `brands/howzero/README.md` + `docs/persona-howzero.md`
- **용감한용팀장**: `brands/braveyong/README.md` (붉은/골드 Accent, Black Han Sans Headline)
- **mkt**: `brands/mkt/README.md` (밝은 회색 + 주황 불릿 + Pretendard Black, 조용한 존댓말 인사이트)
- **집사자**: `brands/zipsaja/README.md` + `brands/zipsaja/workflow.md` + `brands/zipsaja/assets/mascot-*.png` (크림 베이지 + 오렌지 물감 pill + Jua 손글씨 + 마스코트 10포즈)
  - **필수**: `.pill`과 커버 타이틀에 SVG `feTurbulence` + `feDisplacementMap` 물감 효과 적용
  - **고급 (선택)**: `scripts/nano_carousel` 파이프라인으로 Nano Banana 2가 손그림 템플릿(마스코트 + 빈 박스) 생성 → `marker_detector.py`로 좌표 검출 → HTML 텍스트 오버레이 → Puppeteer capture. 생성 불안정성 있어 하이브리드(Nano 성공분 + HTML 폴백) 권장. 상세는 `brands/zipsaja/workflow.md`.
  - **레퍼런스**: `docs/content/carousel-zipsaja-gayang-20260420/` (HTML 11장 `html/` + Nano 7장 `nano/` 분리 보존 구조)
