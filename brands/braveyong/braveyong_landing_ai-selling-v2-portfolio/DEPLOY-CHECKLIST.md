...
### 2026-08-13 (19차) — 🚨 신청서 관문이 결제를 막던 문제 긴급 수정 (commit `현재`)
- `vercel deploy --prod --yes --archive=tgz` → deployment `...c9xn1a6lx` **READY (production)**
- **사고**: 18차 배포 후 사용자 신고 "결제하기가 안 된다". 헤드리스 브라우저로 재현 성공
- 원인 ①: 체크박스를 `formOpened` 로 `disabled` 시켰다. 폼 링크를 안 누른 사용자에게는
  체크박스도 "다음" 버튼도 회색으로 죽어 보인다 — **사용자 눈에는 결제 불능**
- 원인 ②: 폼에 갔다 돌아오면 페이지가 리로드돼 `formOpened` 가 초기화됐다.
  **카톡·인스타 인앱 브라우저는 `target=_blank` 가 웹뷰를 갈아치우므로 복귀 = 리로드**다.
  신청서를 실제로 작성한 사람도 영원히 잠긴 채 루프에 빠졌다
- 수정:
  - 체크박스 `disabled` 제거 — 언제든 체크 가능(자기확인). 이미 폼을 쓴 사람도 바로 진행
  - "다음" 버튼도 `disabled` 대신 눌렀을 때 이유를 안내(회색 죽은 버튼은 고장으로 읽힌다)
  - 링크를 열었다는 사실은 `localStorage` 로 리로드 너머까지 유지. 실패해도 결제를 막지 않는다
  - 서버 관문(`applicationConfirmed`)은 그대로 유지
- 라이브 검증(헤드리스, 모바일 뷰포트): 링크 미클릭 상태에서도 체크박스 활성 →
  체크 → 다음 → **결제 단계 도달**(이름·번호 입력·결제 버튼 존재) · JS 에러 0
- **교훈**: 결제 경로에 `disabled` 를 거는 게이트는 넣지 말 것. 사용자는 이유를 모르고
  "고장"으로 읽는다. 외부 링크로 나갔다 오는 흐름은 **리로드를 전제**로 설계해야 한다
  (인앱 브라우저가 다수). 게이트 상태는 반드시 영속화하고, 실패 시 fail-open
