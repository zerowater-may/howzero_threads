...
- 라이브 검증: `https://gigclass.kr/`·`www`·`/complete` HTTP 200 —
  "3기" 57건 · "8월 22일" 20건 · "7월 25일"·"8월 15일"·"220만원"·"200만원" **0건**,
  JSON-LD `(3기)`, 결제 버튼 "지금 3기 결제하기 — 253만원 (부가세 포함)",
  og-banner 1200x630 HTTP 200, 1280px·320px 가로 오버플로 **0px**
- ⚠️ `npm run test:landing`은 이 배포 이전부터 실패 중 — 7/20 히어로 개편 때 카피가 바뀌었는데
  검증 스크립트(`requiredHeroCopy`)가 안 따라감. hero.tsx는 이번 변경에서 손대지 않음(diff 0). 별건 정리 필요
